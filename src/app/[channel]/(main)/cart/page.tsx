	...
	if (!checkout || checkout.lines.length < 1) {
		return (
			<section className="mx-auto flex max-w-7xl flex-col p-4">
				<h1 className="mt-8 text-3xl font-bold text-primary">Your Shopping Cart is empty</h1>
				<p className="my-12 text-sm text-neutral-500">
					Looks like you haven’t added any items to the cart yet.
				</p>
				<LinkWithChannel
					href="/products"
					className="inline-block max-w-full border border-transparent bg-neutral-900 px-6 py-3 text-center font-medium text-neutral-50 hover:bg-neutral-800 aria-disabled:cursor-not-allowed aria-disabled:bg-neutral-500 sm:px-16"
				>
					Explore products
				</LinkWithChannel>
			</section>
				...
				>
					{checkout.lines.map((item) => (
						<li key={item.id} className="flex py-4">
							<div className="h-30 aspect-[4/6] w-24 flex-shrink-0 overflow-hidden border bg-neutral-50 sm:h-40 sm:w-32">
							...
							<div className="relative flex flex-1 flex-col justify-between p-4 py-2">
								<div className="flex justify-between justify-items-start gap-2">
									<div>
										<LinkWithChannel
