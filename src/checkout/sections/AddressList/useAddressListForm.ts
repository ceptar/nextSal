...
export const useAddressListForm = ({
	...
	interface AddressUpdateSuccessFn {
		(address: OptionalAddress): Promise<void>;
	}
	...
	const onAddressUpdateSuccess: AddressUpdateSuccessFn = async (address: OptionalAddress) =>
		addressListUpdate(
			address,
			addressList.map((existingAddress: AddressFragment) => (existingAddress.id === address?.id ? address : existingAddress)),
		);
